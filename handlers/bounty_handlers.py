...
class BountyHandlers:
    ...
    async def handle_bounty_list(self, player: Player, event: AstrMessageEvent):
        ...
        for bounty in bounties:
            ...
            lines.append(
                f"[{bounty['id']}] {bounty['name']}（{bounty.get('difficulty_name', '未知')}·{bounty.get('category', '任务')}）\n"
                f"  - 目标：完成 {bounty.get('count')} 次 | 时限：{bounty.get('time_limit', 0) // 60} 分钟\n"
                f"  - 奖励：{reward.get('stone', 0):,} 灵石 + {reward.get('exp', 0):,} 修为\n"
                f"  - 说明：{bounty.get('description', '')}"
            )
        lines.append("━━━━━━━━━━━━━━━")
