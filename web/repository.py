...
try:
    ...
    from battle_hp_utils import resolve_player_battle_hp_state
from .common import CONFIG_DIR, load_json_file
...
class WebPreviewRepositoryBase:
    """Web 预览仓库共享基类。"""
    ...
    def __init__(self, db_path: Path):
        ...
        self.bounty_config = load_json_file(
            CONFIG_DIR / "bounty_templates.json",
            {"difficulties": {}, "templates": [], "item_tables": {}},
        )
    ...
    @staticmethod
    def _normalize_config_items(source: Any, fallback_type: str | None = None) -> list[dict[str, Any]]:
        if isinstance(source, dict):
    ...
    def _resolve_boss_battle_hp_for_display(
        ...
        resolved_hp, _, recovery_enabled, cooldown_remaining, _, _ = resolve_player_battle_hp_state(
            current_hp,
            max_hp,
            self._safe_json(user_cd_row["extra_data"], {}),
            now=int(time.time()),
        )
    ...
    def get_shop_preview(self) -> dict[str, list[dict[str, Any]]]:
        pills = []
        pills.extend(self._normalize_config_items(self.pills_config, "pill"))
        pills.extend(self._normalize_config_items(self.exp_pills_config, "exp_pill"))
        pills.extend(self._normalize_config_items(self.utility_pills_config, "utility_pill"))

        equipment = self._normalize_config_items(self.weapons_config, "weapon")
        treasure = self._normalize_config_items(self.items_config)
    ...
    def get_rift_preview(self, user_id: str) -> dict[str, Any]:
        ...
        if next_refresh_raw:
            try:
                remaining_seconds = max(0, int(next_refresh_raw) - int(time.time()))
    ...
    def get_spirit_farm_preview(self, user_id: str) -> dict[str, Any]:
        try:
            from ..managers.spirit_farm_manager import FARM_LEVELS, SPIRIT_HERBS
        except ImportError:
            from managers.spirit_farm_manager import FARM_LEVELS, SPIRIT_HERBS
        ...
        options = [
            ...
            for name, config in SPIRIT_HERBS.items()
        ]
        ...
        for crop in crops_raw if isinstance(crops_raw, list) else []:
            ...
            meta = SPIRIT_HERBS.get(crop_name, SPIRIT_HERBS["灵草"])
    ...
    def get_inventory_preview(self, storage_items: dict[str, Any], pills_inventory: dict[str, Any]) -> dict[str, Any]:
        ...
        for item_name, value in storage_items.items():
            ...
            elif item_type in {"main_technique", "technique", "锻炉石"}:
...
class WebPreviewRepository:
    ...
    当前仍以 `web_preview_server.py` 中的完整仓库实现为准，
    这里只提供稳定入口，后续再逐步把剩余方法继续迁入共享层。
    """
    ...
    def __new__(cls, db_path: Path):
        ...
        return LegacyWebPreviewRepository(db_path)
